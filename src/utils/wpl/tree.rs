//! WPL tree-sitter 辅助逻辑。

use tree_sitter::Parser;

/// 收集需要原样保留的 separator 片段范围。
pub(super) fn collect_separator_ranges(input: &str) -> (Vec<(usize, usize)>, bool) {
...
/// 对 tree-sitter 未直接覆盖的 separator 场景做补充扫描。
...
/// 在已排序的范围列表中查找当前位置对应的 separator 片段。
pub(super) fn find_range_at(ranges: &[(usize, usize)], offset: usize) -> Option<(usize, usize)> {
    if ranges.is_empty() {
        return None;
    }
...
/// 当当前字符位于 separator 起点时，返回整块的结束位置。
pub(super) fn find_separator_block(
    bytes: &[u8],
    start: usize,
    ranges: &[(usize, usize)],
) -> Option<usize> {
...
/// 将字节偏移映射回字符偏移，便于继续按 `char` 维度扫描。
pub(super) fn byte_to_char_index(offsets: &[usize], end: usize, input_len: usize) -> usize {
    if end >= input_len {
        return offsets.len();
    }
...
/// 在 tree-sitter 容错成功时，根据最近有效行推断闭合符号应使用的缩进层级。
pub(super) fn infer_closing_indent(out: &str, indent_unit: usize) -> usize {
    let line = out
        .rsplit('\n')
        .find(|value| {
